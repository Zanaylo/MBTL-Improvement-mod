...
int WantedKind()
{
	return UpscaleFilter::Clamp(g_settings.upscaleFilter);
}
...
{
	return desc.Width == static_cast<unsigned>(GameOffsets::Render::kSceneWidth) &&
		desc.Height == static_cast<unsigned>(GameOffsets::Render::kSceneHeight);
}
