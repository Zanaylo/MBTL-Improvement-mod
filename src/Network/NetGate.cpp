...
{
	return snapshot.hasPeer && snapshot.peer.pending > NetGate::kBusyPending;
}
	...
	const char*& why)
{
	...
	if (!request.probe && (g_verifier == nullptr || !g_verifier(request.to)))
		return Drop(why, "the opponent is not known to run the mod, so nothing is sent to them");

	if (snapshot.ggpoRead && (snapshot.synchronizing || snapshot.peer.state != NetLink::State_Running))
		return Hold(why, "the match connection is still starting");

	if (!snapshot.ggpoRead && snapshot.peer.state == NetLink::State_Disconnected)
		return Hold(why, "the connection to the opponent failed");

	if (LinkBusy(snapshot))
		return Hold(why, "the connection is busy");
