...
UINT HighestMultipleOfSixty(IDirect3D9* d3d9, UINT adapter, const D3DPRESENT_PARAMETERS& parameters, UINT ceiling)
{
	UINT best = 0;
...
bool SmallestListedAtLeast(IDirect3D9* d3d9, UINT adapter, D3DFORMAT format, UINT& width, UINT& height)
{
...
UINT ChooseRefreshRate(IDirect3D9* d3d9, UINT adapter, const D3DPRESENT_PARAMETERS& parameters)
{
	if (g_settings.fullscreenRefreshHz > 0)
		return static_cast<UINT>(g_settings.fullscreenRefreshHz);
	...
	if (!VsyncIsOn(parameters))
		return RateIsListed(d3d9, adapter, parameters, desktop.RefreshRate) ? desktop.RefreshRate
			: parameters.FullScreen_RefreshRateInHz;

	if (desktop.RefreshRate % kTickRateHz == 0 && RateIsListed(d3d9, adapter, parameters, desktop.RefreshRate))
		return desktop.RefreshRate;

	const UINT even = HighestMultipleOfSixty(d3d9, adapter, parameters, desktop.RefreshRate);
	return even != 0 ? even : parameters.FullScreen_RefreshRateInHz;
}

UINT ChooseBackBufferCount(const D3DPRESENT_PARAMETERS& parameters)
{
	if (!g_settings.extraBackBuffer || !VsyncIsOn(parameters))
...
{
	if (!g_settings.disableBackBufferAa || parameters.MultiSampleType == D3DMULTISAMPLE_NONE)
		return;

	LOG("PresentTuning: back buffer multisampling %u -> none", static_cast<unsigned>(parameters.MultiSampleType));
...
void RewriteBackBufferSize(IDirect3D9* d3d9, UINT adapter, D3DPRESENT_PARAMETERS& parameters)
{
	...
	UINT width = static_cast<UINT>(g_settings.presentWidth);
	UINT height = static_cast<UINT>(g_settings.presentHeight);
	...
	if (!parameters.Windowed)
	{
		...
		{
			LOG("PresentTuning: %ux%u is not a mode this adapter lists and nor is anything larger, so the drawing "
				"size is left alone", askedWidth, askedHeight);
			return;
		}
	}
	...
	LOG("PresentTuning: back buffer %ux%u -> %ux%u", parameters.BackBufferWidth, parameters.BackBufferHeight, width,
		height);
...
void RewriteDisplay(IDirect3D9* d3d9, UINT adapter, D3DPRESENT_PARAMETERS& parameters)
{
...
void Rewrite(IDirect3D9* d3d9, UINT adapter, D3DPRESENT_PARAMETERS& parameters)
{
	...
	RewriteDisplay(d3d9, adapter, parameters);
}
...
void PresentTuning::Apply(IDirect3DDevice9* device, D3DPRESENT_PARAMETERS& parameters)
{
	...
	if (d3d9 != nullptr && SUCCEEDED(device->GetCreationParameters(&creation)))
