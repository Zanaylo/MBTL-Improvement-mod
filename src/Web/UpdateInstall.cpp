...
class InstallListener final : public IDeviceListener
{
public:
	void OnPresent(IDirect3DDevice9*) override { UpdateInstall::OnFrame(); }
};

InstallListener g_listener;
...
{
	return _stricmp(name.c_str(), MBTL_IM_ENTRY_DLL) == 0 ||
		_stricmp(name.c_str(), MBTL_IM_UPDATER_EXE) == 0;
}
...
bool ValidateZip(const std::string& path, std::string& outError)
{
	...
	for (const std::string& name : names)
	{
		...
		hasDll = hasDll || _stricmp(name.c_str(), MBTL_IM_ENTRY_DLL) == 0;
	}
...
{
	const std::string wanted = std::string(MBTL_IM_PACKAGE_PREFIX) + release.version + ".zip";
	...
	std::string& outPath, std::string& outError)
{
	...
	body += "InstallRoot=" + GetGameDirectory() + "\r\n";
	...
	body += "EntryDll=" MBTL_IM_ENTRY_DLL "\r\n";
	body += "SteamAppId=" MBTL_IM_STEAM_APP_ID "\r\n";
	body += "GameExe=" + Combine(GetGameDirectory(), MBTL_IM_GAME_EXE) + "\r\n";
	body += "Relaunch=1\r\n";
...
{
	const std::string staged = Combine(stage, MBTL_IM_UPDATER_EXE);

	if (Exists(staged))
		return staged;

	return Combine(GetGameDirectory(), MBTL_IM_UPDATER_EXE);
}

bool PrepareFolders(const std::string& stage, const std::string& downloads, Web::Job& job)
{
	if (CreateDirectoryTree(stage) && CreateDirectoryTree(downloads) &&
		CreateDirectoryTree(Combine(UpdaterRoot(), "logs")))
	{
		return true;
	}
...
bool LaunchUpdater()
{
	...
	const std::string root = GetGameDirectory();
...
void UpdateInstall::OnFrame()
{
	...
	LOG("UpdateInstall: handing over to " MBTL_IM_UPDATER_EXE " and closing the game");
	ExitProcess(0);
}

IDeviceListener* UpdateInstall::Listener()
{
	return &g_listener;
}
