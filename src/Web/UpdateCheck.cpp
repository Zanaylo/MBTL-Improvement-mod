...
char g_version[32] = {};
char g_page[256] = MBTL_IM_RELEASE_PAGE;
...
void Publish(const GitHubRelease::Release& release, bool newer)
{
	...
	strncpy_s(g_page, release.page.empty() ? MBTL_IM_RELEASE_PAGE : release.page.c_str(),
		_TRUNCATE);
...
DWORD WINAPI Run(LPVOID)
{
	...
	else
	{
		...
		LOG("update check: %s is the latest, this is %s%s", release.version.c_str(),
			MBTL_IM_VERSION, newer ? " - an update is available" : "");
	}
