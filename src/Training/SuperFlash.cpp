...
SuperFlash::Reading SuperFlash::Read(const PlayerState::State* states, const bool* valid, int count) const
{
	Reading reading = {};
	reading.owner = -1;
	...
	for (int player = 0; player < count && player < kPlayers; ++player)
	{
		...
		if ((state.moveCode[3] & Codes::kAnten) != 0)
		{
		...
		if ((state.moveCode[2] & Codes::kEnemyAntenStop) == 0)
			continue;
...
void SuperFlash::Advance(const Reading& reading)
{
	...
	if (!m_freezing || m_owner < 0 || m_frames[m_owner] >= kMaxFrames)
		return;
