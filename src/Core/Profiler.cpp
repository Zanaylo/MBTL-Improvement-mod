...
void Profiler::BuildSummary(char* out, size_t size)
{
	...
	snprintf(out, size,
		...
		"tuning     %s\n"
		"options    timer=%d throttling=%d displayTuning=%d extraBuffer=%d\n"
		...
		g_settings.timerResolution ? 1 : 0, g_settings.powerThrottlingOptOut ? 1 : 0,
		g_settings.displayTuning ? 1 : 0, g_settings.extraBackBuffer ? 1 : 0,
