...
constexpr const char* kStagePrefix = "bg\\bg";
...
std::string ReadDescription(const std::string& path, const char* key, const char* fallback)
{
	char value[192] = {};
...
std::vector<std::string> Stored(const char* key)
{
	...
	GetPrivateProfileStringA(kSection, key, "", value.data(), kListBytes, Settings::IniPath().c_str());
...
int PlaceOf(const std::vector<std::string>& order, const std::string& id)
{
	...
	return found == order.end() ? static_cast<int>(order.size()) : static_cast<int>(found - order.begin());
}

int StageIn(const FileIndex& files)
{
	const size_t length = std::strlen(kStagePrefix);

	for (const FileIndex::Map::value_type& entry : files.Entries())
	{
		if (entry.first.compare(0, length, kStagePrefix) != 0)
			continue;

		const int number = atoi(entry.first.c_str() + length);
...
void Weigh()
{
	...
	for (Entry& pack : g_packs)
	{
		...
		if (pack.info.beaten != 0)
			LOG("ModPacks: %d file(s) of %s are already answered by %s", pack.info.beaten, pack.info.name.c_str(),
				pack.info.beatenBy.c_str());
	}
}
...
void Take(const std::string& folder, const std::string& id, const std::vector<std::string>& off)
{
	...
	pack.info.name = pack.info.described ? ReadDescription(description, "Name", id.c_str()) : id;
	pack.info.author = pack.info.described ? ReadDescription(description, "Author", "") : std::string();
	pack.info.version = pack.info.described ? ReadDescription(description, "Version", "") : std::string();
	pack.info.note = pack.info.described ? ReadDescription(description, "Note", "") : std::string();
...
void ModPacks::Scan()
{
	...
	if (search != INVALID_HANDLE_VALUE)
	{
		do
		{
			if (found.cFileName[0] == '.' || (found.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0)
				continue;
	...
	std::stable_sort(g_packs.begin(), g_packs.end(), [&order](const Entry& a, const Entry& b)
	{
...
int ModPacks::FileCount()
{
	...
	for (const Entry& pack : g_packs)
		files += pack.info.enabled ? pack.info.files : 0;
...
bool ModPacks::Install(const std::string& zip, char* status, int statusSize)
{
	...
	{
		strncpy_s(status, statusSize, "could not read that zip", _TRUNCATE);
		return false;
	}
	...
	{
		sprintf_s(status, statusSize, "could not unpack the zip: %s", note);
		return false;
	}

	Scan();

	sprintf_s(status, statusSize, "%s installed, %d file(s)", inside.empty() ? LeafOf(zip).c_str() : inside.c_str(),
		files);
...
int ModPacks::StageOwner(int number, int except)
{
	...
	{
		if (i != except && g_packs[i].info.enabled && g_packs[i].info.stage == number)
			return i;
	}
