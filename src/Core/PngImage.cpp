...
bool Unfilter(const std::vector<uint8_t>& raw, int width, int height, int channels, std::vector<uint8_t>& out)
{
	...
	for (int row = 0; row < height; ++row)
	{
		...
		for (size_t i = 0; i < stride; ++i)
		{
			...
			const int corner = up == nullptr || i < static_cast<size_t>(channels) ? 0 : up[i - channels];
...
void Sample(const uint8_t* pixel, int colour, const std::vector<uint8_t>& palette, const std::vector<uint8_t>& clear,
	uint8_t* out)
{
	if (colour == 3)
	{
...
bool Expand(const std::vector<uint8_t>& lines, int width, int height, int channels, int colour,
	const std::vector<uint8_t>& palette, const std::vector<uint8_t>& clear, std::vector<uint8_t>& out)
{
...
bool PngImage::Decode(const std::vector<uint8_t>& blob, int& outWidth, int& outHeight, std::vector<uint8_t>& outBgra)
{
	if (blob.size() < kSignature + kChunkFrame + kHeaderBody || memcmp(blob.data(), kMagic, kSignature) != 0)
		return false;
	...
	if (!Deflate::Inflate(stream.data() + kZlibHeader, stream.size() - kZlibHeader, raw, expected) ||
		!Unfilter(raw, width, height, channels, lines) || !Expand(lines, width, height, channels, colour, palette, clear, outBgra))
	{
		return false;
	}
