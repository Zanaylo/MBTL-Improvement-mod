...
bool LookIsOn()
{
	return g_settings.lookEnabled && !PostChain::IsLookNeutral();
}
...
int CollectStages(int* out)
{
	...
	if (AntiAlias::Clamp(g_settings.antiAliasing) != AntiAlias::Level_Off)
		out[count++] = Stage_Fxaa;

	if (g_settings.bloomEnabled && g_settings.bloomIntensity > 0)
		out[count++] = Stage_Bloom;

	if (LookIsOn())
		out[count++] = Stage_Look;

	if (SharpenMode::Clamp(g_settings.sharpenMode) != SharpenMode::Kind_Off &&
		g_settings.sharpenStrength > 0)
	{
...
IDirect3DPixelShader9* ShaderFor(IDirect3DDevice9* device, int stage)
{
	...
	case Stage_Sharpen:
		return g_sharpen.Ensure(device, SharpenMode::GetBytecode(g_settings.sharpenMode))
...
void SetStageConstants(IDirect3DDevice9* device, int stage, unsigned width, unsigned height)
{
	...
	case Stage_Fxaa:
	{
		const AntiAlias::Tuning tuning = AntiAlias::GetTuning(g_settings.antiAliasing);
	...
	case Stage_Sharpen:
		...
		FullScreenQuad::SetConstant(device, 1, Positive(g_settings.sharpenStrength), 0.0f, 0.0f,
			0.0f);
		return;

	case Stage_Look:
		FullScreenQuad::SetConstant(device, 0, Signed(g_settings.lookBrightness),
			Signed(g_settings.lookContrast), static_cast<float>(g_settings.lookGamma) / 100.0f,
			Signed(g_settings.lookSaturation));
		FullScreenQuad::SetConstant(device, 1, Signed(g_settings.lookVibrance),
			Signed(g_settings.lookTemperature), Positive(g_settings.lookVignette),
			Positive(g_settings.lookScanlines));
		FullScreenQuad::SetConstant(device, 2, static_cast<float>(width),
			static_cast<float>(height), g_settings.lookDither ? 1.0f : 0.0f, 0.0f);
		return;
	...
	const D3DSURFACE_DESC& desc)
{
	...
	FullScreenQuad::SetConstant(device, 1, Positive(g_settings.bloomThreshold), 0.25f, 0.0f, 0.0f);
	...
	FullScreenQuad::SetConstant(device, 0,
		static_cast<float>(g_settings.bloomIntensity) / 50.0f, 0.0f, 0.0f, 0.0f);
...
{
	return g_settings.lookBrightness == 0 && g_settings.lookContrast == 0 &&
		g_settings.lookSaturation == 0 && g_settings.lookVibrance == 0 &&
		g_settings.lookTemperature == 0 && g_settings.lookVignette == 0 &&
		g_settings.lookScanlines == 0 && g_settings.lookGamma == 100 && !g_settings.lookDither;
}
...
{
	g_settings.lookBrightness = 0;
	g_settings.lookContrast = 0;
	g_settings.lookSaturation = 0;
	g_settings.lookVibrance = 0;
	g_settings.lookTemperature = 0;
	g_settings.lookVignette = 0;
	g_settings.lookScanlines = 0;
	g_settings.lookGamma = 100;
	g_settings.lookDither = false;
...
{
	g_settings.antiAliasing = AntiAlias::Level_Off;
	g_settings.sharpenMode = SharpenMode::Kind_Off;
	g_settings.sharpenStrength = 0;
	g_settings.bloomEnabled = false;
	g_settings.lookEnabled = false;
