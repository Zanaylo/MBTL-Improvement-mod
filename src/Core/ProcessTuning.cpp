...
SetProcessInformation_t ResolveSetProcessInformation()
{
	...
	resolved = reinterpret_cast<SetProcessInformation_t>(GetProcAddress(kernel32, "SetProcessInformation"));
	return resolved;
}
...
bool WriteThrottlingState(ULONG controlMask)
{
	...
	return setProcessInformation(GetCurrentProcess(), kProcessPowerThrottling, &state, sizeof(state)) != FALSE;
}
...
void ProcessTuning::Initialize()
{
	...
	LOG("Process tuning applied (timer period held=%d, throttling opt-out=%d)", g_timerPeriodHeld ? 1 : 0,
		g_throttlingOptedOut ? 1 : 0);
}
...
void ProcessTuning::Apply()
{
	...
	if (g_settings.timerResolution)
		HoldTimerPeriod();
	else
		ReleaseTimerPeriod();

	if (g_settings.powerThrottlingOptOut)
		OptOutOfThrottling();
	else
