...
void OnLobbyEnter(const uint8_t* data)
{
	...
	const int response = I32(data, 16);

	LobbyWatch::Entered(lobby, response);
	NetLog::Write("steam: entered lobby %llu, response %d, locked %u", static_cast<unsigned long long>(lobby), response,
		data[12]);
}
...
void OnLobbyChatUpdate(const uint8_t* data)
{
	const uint64_t lobby = U64(data, 0);
	const uint64_t member = U64(data, 8);
	const int stateChange = I32(data, 24);

	LobbyWatch::MemberChanged(lobby, member, stateChange);

	NetLog::Write("steam: lobby %llu member %llu state 0x%02x, changed by %llu", static_cast<unsigned long long>(lobby),
		static_cast<unsigned long long>(member), stateChange, static_cast<unsigned long long>(U64(data, 16)));
}
...
bool SteamWatch::Register()
{
	...
	{
		NetLog::Write("steam: SteamAPI_RegisterCallback is missing, Steam events are not watched");
		return false;
	}
