...
constexpr int kOutboxSlots = 8;
constexpr int kInboxSlots = 8;
...
bool Enqueue(bool toPeer, bool probe, uint64_t to, const void* data, int size, DWORD ttlMs, const char* label)
{
	...
	for (Outgoing& slot : g_outbox)
	{
		...
		slot.request = { toPeer, probe, to, GetTickCount(), ttlMs, size };
...
bool ModChannel::ProbePeer(const void* data, int size, DWORD ttlMs, const char* label)
{
	return Enqueue(true, true, NetLink::Peer(), data, size, ttlMs, label);
}
